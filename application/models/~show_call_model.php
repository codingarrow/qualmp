  ...
  $qmp_viewandrateuser = 'qmp_csvviewandrateuser';
	...
	$morewhere = "";
	      if  (isset($_REQUEST["campaign"]))
			...
			$morewhere = " AND campaign_id IN (".ew_GetArrPost($_REQUEST["campaign"],""). ")"; 
		      }
	      if  (isset($_REQUEST["disps"]))
		      {
			//echo "user dispositions:".ew_GetArrPost($_REQUEST["disps"],"q")."<br>";
			$morewhere .= " AND disposition_code IN (".ew_GetArrPost($_REQUEST["disps"],"q"). ")"; 
		      }
	      if  (isset($_REQUEST["sdisps"]))
		      {
			//echo "system dispositions:".ew_GetArrPost($_REQUEST["sdisps"],"q")."<br>";
			$morewhere .= " AND system_disposition IN (".ew_GetArrPost($_REQUEST["sdisps"],"q"). ")"; 
		      }
	      ...
	      if  (isset($_REQUEST["agentss"]))
			...
			$morewhere .= " AND user_id IN (".ew_GetArrPost($_REQUEST["agentss"],"q"). ")"; 
		      }
		      /*
	      ...
	      if  (isset($_REQUEST["phone"]))
			...
			$morewhere .= " AND phone IN (".ew_QuotedValue($_REQUEST["phone"],EW_DATATYPE_STRING). ")"; 
		      }
	      if  (isset($_REQUEST["vlogfname"]))
		      {
			//echo "filename:".ew_GetArrPost($_REQUEST["vlogfname"],"q")."<br>";
			$morewhere .= " AND recording_file_url IN (".ew_QuotedValue($_REQUEST["vlogfname"],EW_DATATYPE_STRING). ")"; 
		      }
	      if  (isset($_REQUEST["callid"]))
		      {
			//echo "callid:".ew_GetArrPost($_REQUEST["callid"],"q")."<br>";
			$morewhere .= " AND call_id similar to '%".ew_QuotedValue($_REQUEST["callid"],"")."%'"; 
		      }
	      if  (isset($_REQUEST["calltime"]))
		      {
			//echo "call>:".ew_GetArrPost($_REQUEST["calltime"],"q")."<br>";
			$morewhere .= " AND (EXTRACT(SECOND FROM call_originate_time)<".ew_QuotedValue($_REQUEST["calltime"],"").")";
		      }
	      if  (isset($_REQUEST["calltime1"]))
		      {
			//echo "call<:".ew_GetArrPost($_REQUEST["calltime1"],"q")."<br>";
			$morewhere .= " AND (EXTRACT(SECOND FROM call_originate_time)>".ew_QuotedValue($_REQUEST["calltime1"],"").")";
		      }
	return  $morewhere;
	}
