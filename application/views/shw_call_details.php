			...
			if ( strlen($call_id) < 3 )
			  {
                        ...
                        //echo '<br />' . $recording_file_url . " here";
                            ...
                            //echo $this->session->userdata('purge_userDIR');
		             ...
		             $FP=fopen ( "$asteriskip/dacx/fileoperation?fileoperation=get&componentType=com-drishti-dacx-core-callmanager-asterisk-IAsteriskComponent&fileonserver=$build_filepath&ingain=1&outgain=1",'r')  ;
                                if (!$FP) {
                                    echo "<p>Unable to open remote file for writing contact Admin to resolve this issue.\n";
                                    //redirect here or show modal or something whatever
                                }
                                else
                                {
                                    //!!echo "<p>FP value. ".$FP."\n";
                                    //$this->purgeaudioFile();
                                    //$this->delete_files("/dacx/ameyo/acp/web/vl/dacx/var/ameyo/dacxdata/voicelogs". $build_filepath."");
                                    
                                    //!!if (is_dir($build_filepath) === true)
                                    //!!{
                                        //return unlink($build_filepath);
                                        //!!$mk = mkdir($build_filepath, 0777) ;
                                    //!!}
                                    
                                    /*
                                      play?filepath=/dacx/var/ameyo/dacxdata/voicelogs/2014-08-12/EBDFajardo/13073212334_EBDFajardo_2014-08-12-17-26-51
                                      filename=61732636170_EBDZamora_2014-08-10-20-14-21
                                      asteriskip=208.80.180.209
                                      callhistoryid=
                                      ingain=1
                                      outgain=1
                                      */
                                    
                                    //echo $filepathnowav . '<br />' . 
                                    
                                    //replace the unwanted string to minimize dir count ®
                                    $filepathnowav = str_replace("dacx/var/ameyo/dacxdata/","",$filepathnowav);
                                    $dirs = explode(DIRECTORY_SEPARATOR , $filepathnowav);
                                    $count = count($dirs);
                                    //!!$path = 'vl/';
                                    /*
                                    $path = '/dacx/ameyo/acp/web/vl';
                                    
                                    previous path needs security credential browser acceptance on user side which can
                                    often be irksome change code to the voicelog audio path of qmp instead
                                    */
                                    $path = '/dacx/ameyo/crm/html/qmp';
                                    
                                    
                                    //!!$path = '/dacx/ameyo/acp/web/vl/dacx/var/ameyo/dacxdata/voicelogs';
                                    
                                    //!!$path = '/dacx/ameyo/acp/web/vl/dacx/var/ameyo/dacxdata/voicelogs';
                                    //!!$path = $_SERVER['DOCUMENT_ROOT']."/qmp";
                                    
                                    //mkdir($path . $build_filepath,0777,true);
                                    /* ------------------------------------------------------
                                    if (!mkdir($path . $build_filepath,0777,true)) {
                                        die('Failed to create folders...');
                                    }
                                       ------------------------------------------------------*/

                                    /*
                                        /dacx/ameyo/crm/html/qmp/voicelogs
                                        
                                        /dacx/ameyo/crm/html/qmp/voicelogs/2014-08-26
                                        
                                        /dacx/ameyo/crm/html/qmp/voicelogs/2014-08-26/EBDCarandang
                                    */
                                    for ($i = 0; $i < $count - 1 ; ++$i) {
                                    ...
                                    /*
                                    echo $_SERVER['DOCUMENT_ROOT']."/qmp/voicelogs".$build_filepath;
                                    $mk = mkdir($_SERVER['DOCUMENT_ROOT']."/qmp/voicelogs".$build_filepath);
                                    if($mk)
                                     {
                                        echo 'created <br/>';
                                     }
                                    /voicelogs/dacx/var/ameyo/dacxdata/voicelogs/2014-08-18/EBDZamora/01161419312319_EBDZamora_2014-08-18-19-44-59
                                    
                                    $path = $_SERVER['DOCUMENT_ROOT']."/qmp/dacx/var/ameyo/dacxdata/voicelogs";
                                    $structure = $path . $build_filepath;
                                    ...
                                    */
                                    ...
                                    //!!if($mk)
                                    //!! {
                                       //!!echo $build_filepath.' dir created';
                                       //----------------------------------------------------------------------
                                       /*
                        filepathnowav - dacx/var/ameyo/dacxdata/voicelogs/2014-08-26/EBDCarandang/6466380018_EBDCarandang_2014-08-26-18-13-14
                        field         - dacx/var/ameyo/dacxdata/voicelogs/2014-08-26/EBDCarandang/6466380018_EBDCarandang_2014-08-26-18-13-14.wav
                        evaluated web - /dacx/ameyo/crm/html/qmp/voicelogs/dacx/var/ameyo/dacxdata/voicelogs/2014-08-26/EBDCarandang/dacx/var/ameyo/dacxdata/voicelogs/2014-08-26/EBDCarandang/6466380018_EBDCarandang_2014-08-26-18-13-14.wav
                        FTP path      - /dacx/ameyo/crm/html/qmp/voicelogs/dacx/var/ameyo/dacxdata/voicelogs/2014-08-26/EBDCarandang
                                        
                                       echo $path . '<br />' . $filepathnowav . '<br />';
                                       echo strtolower(array_shift(explode("/",$_SERVER['SERVER_PROTOCOL'])))."://".$_SERVER['HTTP_HOST'] . '<br />';
                                        **/
                                       $fppathnowav = basename($filepathnowav); //$filepathnowav
                                       //$newpathtoaudio = strtolower(array_shift(explode("/",$_SERVER['SERVER_PROTOCOL'])))."://".$_SERVER['HTTP_HOST'] . '/' . $fppathnowav . '.wav';
                                       $newpathtoaudio = base_url() . $filepathnowav . '.wav';
                                       
                                            //$FP1=fopen ('/dacx/ameyo/acp/web/vl/' . $filepathnowav . '.wav' ,'w')  ;
                                            $FP1=fopen ($path . '/' . $fppathnowav . '.wav' ,'w')  ;
                                            if ($FP1 == null)
                                            {
			                    ...
			                    fclose($FP1) ;
                                     ...
                                     //!!else
                                     //!!{
                                       //echo $build_filepath. 'not created';   
                                     //!!}
                                }
			        fclose($FP) ;
                            }
